...
const BotMemory = require("./memory/memory.js");
const BotHooks = require("./botHooks.js");
const BotPermission = require("./permissions.js");
...
class Bot {
    ...
    applyConfigToCommand(command) {
        let pluginOverrides = this.config.commandRequiredPermissionOverrides[
            this.memory.keyCreator.createLocationKey_plugin(command.pluginName)
        ];
    ...
    getConfig_plugin(namespace) {
        return this.config.get(this.memory.keyCreator.createLocationKey_plugin(namespace));
    }
