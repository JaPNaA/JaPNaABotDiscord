...
class BotPermissions {
    ...
    getPermissions_role_channel(roleId, serverId, channelId) {
        ...
        if (channelId) {
            permissions.importCustomPermissions(
                this.memory.get(this.memory.keyCreator.strings.permissionsNamespace,
                    this.memory.keyCreator.createLocationKey_role_channel(serverId, roleId, channelId)
                ));
        }

        permissions.importCustomPermissions(
            this.memory.get(this.memory.keyCreator.strings.permissionsNamespace,
                this.memory.keyCreator.createLocationKey_role_server(serverId, roleId)
            ));
    ...
    getPermissions_global(userId) {
        ...
        permissions.importCustomPermissions(
            this.memory.get(this.memory.keyCreator.strings.permissionsNamespace, this.memory.keyCreator.createLocationKey_user_global(userId))
        );
        return permissions;
    }
    ...
    getPermissions_channel(userId, serverId, channelId) {
        ...
        if (serverId) {
            permissions.importCustomPermissions(
                this.memory.get(this.memory.keyCreator.strings.permissionsNamespace, this.memory.keyCreator.createLocationKey_user_server(serverId, userId))
            );
        }
        ...
        if (channelId) {
            permissions.importCustomPermissions(
                this.memory.get(this.memory.keyCreator.strings.permissionsNamespace,
                    this.memory.keyCreator.createLocationKey_user_channel(serverId, userId, channelId)
                )
            );
        }
    ...
    editPermissions_user_channel(userId, channelId, permissionName, value) {
        ...
        let customPerms = this.memory.get(this.memory.keyCreator.strings.permissionsNamespace,
            this.memory.keyCreator.createLocationKey_user_channel(serverId, userId, channelId)
        );
        ...
        let locationKey = this.memory.keyCreator.createLocationKey_user_channel(serverId, userId, channelId);
        if (customPerms.length) {
            this.memory.write(this.memory.keyCreator.strings.permissionsNamespace,
    ...
    editPermissions_user_server(userId, serverId, permissionName, value) {
        let customPerms = this.memory.get(this.memory.keyCreator.strings.permissionsNamespace,
            this.memory.keyCreator.createLocationKey_user_server(serverId, userId)
        );
        ...
        customPerms = permissions.getCustomPermissions();
        let locationKey = this.memory.keyCreator.createLocationKey_user_server(serverId, userId);
        if (customPerms.length) {
            this.memory.write(this.memory.keyCreator.strings.permissionsNamespace,
                locationKey, customPerms, true
            );
    ...
    editPermissions_role_channel(roleId, channelId, permissionName, value) {
        ...
        customPerms = permissions.getCustomPermissions();
        let locationKey = this.memory.keyCreator.createLocationKey_role_channel(serverId, roleId, channelId);
        if (customPerms.length) {
            this.memory.write(this.memory.keyCreator.strings.permissionsNamespace,
                locationKey, customPerms, true
            );
    ...
    editPermissions_role_server(roleId, serverId, permissionName, value) {
        ...
        customPerms = permissions.getCustomPermissions();
        let locationKey = this.memory.keyCreator.createLocationKey_role_server(serverId, roleId);
        if (customPerms.length) {
            this.memory.write(this.memory.keyCreator.strings.permissionsNamespace,
                locationKey, customPerms, true
            );
    ...
    editPermissions_user_global(userId, permissionName, value) {
        let customPerms = this.memory.get(this.memory.keyCreator.strings.permissionsNamespace,
            this.memory.keyCreator.createLocationKey_user_global(userId)
        );
        ...
        customPerms = permissions.getCustomPermissions();
        let locationKey = this.memory.keyCreator.createLocationKey_user_global(userId);
        if (customPerms.length) {
            this.memory.write(this.memory.keyCreator.strings.permissionsNamespace,
                locationKey, customPerms, true
            );
