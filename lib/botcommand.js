...
class BotCommand {
    ...
    test(commandEvent) {
        ...
        if (
            ...
            let permissions = this.bot.permission.getPermissions_channel(commandEvent.userId, commandEvent.serverId, commandEvent.channelId);
    ...
    sendError(commandEvent, argString, error) {
        ...
        errorStack = errorStack.split(__dirname).join(".");
        let message =
            "```An error occured\n" + errorStack +
